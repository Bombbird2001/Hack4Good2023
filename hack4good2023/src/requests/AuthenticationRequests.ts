...
/**
 * Creates an account with the supplied credentials.
 * @param username  The username to log in with.
 * @param displayName The display name to create account with.
 * @param onError   The function to be executed if login fails.
 * @param onSuccess The function to be executed if login succeeds.
 */
export const createAccount = (username: string, displayName: string,
                      onError: (reason: string) => void, onSuccess: (username: string, displayName: string) => void) => {
    let xhr = new XMLHttpRequest()
    xhr.open("POST", "http://localhost:4149/createAccount", true)
    setContentHeader(xhr)
    xhr.onload = (_e) => {
        if (xhr.readyState === 4) {
            if (xhr.status === 200) {
    ...
    let creds = {"username": username, "displayName": displayName}
    xhr.send(JSON.stringify(creds))
}
